    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 fade-in min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex gap-4">
           <div className="relative w-64"><Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" /><input type="text" placeholder="Search by UID..." className="input pl-10" /></div>
      ...
      <div className="card p-0 overflow-hidden">
         <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-[var(--border)] text-[10px] font-bold text-gray-600 uppercase tracking-[2px]">
               <tr>
                  ...
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                     <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                           ...
                           <div><p className="font-bold text-sm text-[var(--ink)]">{u.n}</p><p className="text-[10px] text-gray-600 font-bold uppercase">UID: {Math.floor(Math.random()*10000)}</p></div>
                        </div>
                     </td>
                     ...
                     <td className="px-6 py-4">
                        <div className="flex gap-2">
                           <button onClick={() => toast.success('Moderation Panel Opened')} className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-gray-600 hover:text-[var(--ink)]"><Shield size={14} /></button>
                           <button onClick={() => toast.error('User access restricted temporarily')} className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-gray-600 hover:text-red-500"><UserX size={14} /></button>
                        </div>
                     </td>
                  </tr>
               ))}
